...
//        file.renameTo(newFile);
    ...
    public static FilesDTO  spesifyFilePath(FilesDTO filesDto) {
        ...
        System.out.println("FileHandler - uploadDir: " + uploadDir);
        ...
        try {
            ...
            if(filesDto != null && !filesDto.isEmpty()) {
                for (FilesDTO fileDto : filesDto) {
                    ...
                    fileData.put("fileType", fileDto.getFileExtension()); // 파일 확장자
                    System.out.println("ai fileData : " + fileData);
            ...
            String pythonApiUrl = "http://localhost:5000/apiCompile";
    ...
    public static List<Map<String, Object>> saveResultCSV(List<Map<String, Object>> compileWordsJson, String saveCsv) {
        ...
        try (FileOutputStream fos = new FileOutputStream(saveCsv);
            ...
            // "단어 목록" 리스트 가져오기
            System.out.println("csv 파일로 변환할 데이터: " + compileWordsJson);

            // 입력 데이터 확인
            if (compileWordsJson == null || compileWordsJson.isEmpty()) {
                throw new RuntimeException("입력 데이터가 비어 있습니다.");
            }
            System.out.println("입력 데이터: " + compileWordsJson);
