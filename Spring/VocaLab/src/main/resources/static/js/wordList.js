...
$('.word-table').on('mousedown', 'td', function (event) {
    ...
    // 디버깅 로그
    console.log('Word:', word, 'Type:', type, 'Meaning:', meaning, 'Example:', example);
...
$('#btnTest').on('click', function () {
    ...
    if (selectedCount < 20) {
        ...
        // 테스트 시작 시 실행할 로직
        console.log('선택된 단어 수:', selectedCount); // 디버깅용 로그
        alert('테스트를 시작합니다!');
    }
});


$('#btnWrite, #btnNews').on('click', function () {
    alert($(this).text() + ' 기능을 준비 중입니다.');
});
$(document).ready(function () {
