    ...
    $('.word-table').on('mousedown', 'td', function (event) {
        ...
        if (event.shiftKey && lastClickedIndex !== null) {
            ...
            if ($(this).hasClass('highlight')) {
                ...
                console.log('Word added:', { word, type, meaning, example });
            }
        }
...
// 컨텐츠 버튼 공통 로직을 처리하는 함수
    function contentButtonClick(pageType) {
        ...
        console.log('Converted Words Array:', selectedWordsArray);
        ...
        if (pageType === 'wordTest') {
            ...
            $('.test-type-btn').off('click').on('click', function() {
                ...
                // 테스트 생성 API 호출
                $.ajax({
                    url: '/contents/wordTest/generate',
                    method: 'POST',
                    contentType: 'application/json',
            ...
            const url = `/WordBook/${pageType}?wordBookId=${wordBookId}`;
            console.log('Redirecting to:', url);

            // 페이지 이동 전에 URL이 유효한지 확인
            if (pageType && ['Writing', 'News'].includes(pageType)) {
